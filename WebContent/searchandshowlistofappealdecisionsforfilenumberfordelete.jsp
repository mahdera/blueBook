				...
				while(appealDecisionItr.hasNext()){
					...
					%>
						<tr>
							...
							<td>
								<a href="#.jsp" onclick="deleteThisAppealDecision(<%=appealDecision.getId() %>);">
									<label id="lbldelete"><%						
										label = Label.getLabelUsingLabelId("lbldelete");
										out.print(lblDelete != null ? lblDelete : label.getLabelCaption());
									%></label>
...
<script type="text/javascript">
	function deleteThisAppealDecision(id){
		var langId = "<%=langId %>";
		var displayMsg = "Are you sure you want to delete this appeal decision?";
		if(langId == 1){
			displayMsg = "ይህን የአቤቱታ ውሣኔ መረጃ ለማስወገድ ፈልገዋል። እንግጠኛ ነዎት?";
		}
		if(window.confirm(displayMsg)){
			var dataString = "id="+id;
			var divId = "subTabDetailDiv";
			$.ajax({
			    url: 'deleteappealdecision.jsp',		
			    data: dataString,
			    type:'POST',
