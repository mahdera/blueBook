...
<%
	...
	if(!caseList.isEmpty()){
				...
				while(caseItr.hasNext()){
					...
					%>
						<tr>
							...
							<td align="center">
								<a href="#.jsp" onclick="showEditFieldsOfThisCase(<%=c.getId() %>);"><label id="lbledit"><%
	 			...
	 			%></label></label></a> |
								<a href="#.jsp" onclick="hideEditFieldsOfThisCase(<%=c.getId() %>);"><label id="lblhide"><%
		...
		%>
		<p class="msg warning"><label id="lblnoinformationfound">There is no information found</label></p>
		<%
	}
%>
<%
...
<script type="text/javascript">	
	function showEditFieldsOfThisCase(caseId){
		var divId = "caseEditDiv"+caseId;
		$('#'+divId).load('showeditfieldsofthiscase.jsp?caseId='+caseId);		
	}
	
	function hideEditFieldsOfThisCase(caseId){
		var divId = "caseEditDiv"+caseId;
		$('#'+divId).html('');
	}
</script>