				...
				<%
					...
					while(investigationItr.hasNext()){
						...
						<tr>
							...
							<td>
								<a href="#.jsp" onclick="deleteThisInvestigation(<%=investigation.getId() %>);">
									<label id="lbldelete">
										<%											
											label = Label.getLabelUsingLabelId("lbldelete");
											out.print(lblDelete != null ? lblDelete : label.getLabelCaption());				
...
<script type="text/javascript">
	function deleteThisInvestigation(id){
		var langId = "<%=langId %>";
		var displayMsg = "Are you sure you want to delete this investigation record?";
		if(langId == 1){
			displayMsg = "ይህን የምርመራ ሂደት መረጃ ለማስወገድ ፈልገዋል። እንግጠኛ ነዎት?";
		}
		if(window.confirm(displayMsg)){
			var fileNumber = "<%=fileNumber%>";
			var dataString = "id="+id;
			$.ajax({
			    url: 'deleteinvestigation.jsp',		
			    data: dataString,
			    type:'POST',
			    success:function(response){		    		
			    		$('#investigationDiv').load('searchandshowlistofinvestigationsforfilenumberfordelete.jsp?fileNumber='+fileNumber);					
			    },
