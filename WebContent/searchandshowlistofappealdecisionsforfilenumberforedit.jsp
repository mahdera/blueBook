				...
				while(appealDecisionItr.hasNext()){
					...
					%>
						<tr>
							...
							<td>
								<a href="#.jsp" onclick="showEditFieldsOfThisAppealDecision(<%=appealDecision.getId() %>);"><label id="lbledit"><%						
						label = Label.getLabelUsingLabelId("lbledit");
						out.print(lblEdit != null ? lblEdit : label.getLabelCaption());
					%></label></a>
								|
								<a href="#.jsp" onclick="hideDiv(<%=appealDecision.getId() %>);"><label id="lblhide"><%
					...
					%></label></a>
						...
						<%
							String divId = "appealDecisionEditDiv" + appealDecision.getId();
						%>
						<tr>
							<td colspan="5">
								<div id="<%=divId%>"></div>
...
<script type="text/javascript">
	function showEditFieldsOfThisAppealDecision(id){
		var divId = "appealDecisionEditDiv" + id;
		$('#'+divId).load("showeditfieldsofthisappealdecision.jsp?id="+id);
	} 
