...
<table border="0" width="100%">
	<tr style="background:#eee;font-size:12pt;font-style:bolder">
		...
		<td><label id="lbldisability"><%
		 			...
		 			%></label></td>
		<td><label id="lbldetails"><%
						String lblDetails = LanguageTranslation.getLabelCaptionInTheSelectedLanguage(langId,"lbldetails");
						label = Label.getLabelUsingLabelId("lbldetails");
		 				out.print(lblDetails != null ? lblDetails : label.getLabelCaption());
		 			%></label></td>
	</tr>
	<%
		...
		while(applicantItr.hasNext()){
			...
			<tr>
				...
				<td>
					<a href="#.jsp" onclick="showAddressDetailOfThisApplicant(<%=applicant.getId() %>);">
						<label id="lbldetail">
							<%
								String lblDetail = LanguageTranslation.getLabelCaptionInTheSelectedLanguage(langId,"lbldetail");
								label = Label.getLabelUsingLabelId("lbldetail");
				 				out.print(lblDetail != null ? lblDetail : label.getLabelCaption());
				 			%>
						</label>
					</a> |
					<a href="#.jsp" onclick="hideAddressDetailOfThisApplicant(<%=applicant.getId() %>);">
						<label id="lblhide">
							<%
			...
			<%
				String uniqueId = "detailDiv"+applicant.getId();
			%>
			<tr>
				<td colspan="10">
					<div id="<%=uniqueId %>"></div>
...
<script type="text/javascript">
	function showAddressDetailOfThisApplicant(applicantId){
		var divId = "detailDiv"+applicantId;
		$('#'+divId).load('showaddressdetailofthisapplicant.jsp?applicantId='+applicantId);		
	}
	
	function hideAddressDetailOfThisApplicant(applicantId){
		var divId = "detailDiv"+applicantId;
		$('#'+divId).html('');
	}
</script>