...
function RenderCssCal(bNewCal)
{
	...
	for (j = 1; j <= Cal.GetMonDays(); j += 1)
	{
		...
		{
			strCell = GenCell(j, false, DisableColor, true);//Before today's date is not clickable
		}
		//if End Year + Current Year = Cal.Year. Disable.
		else if (Cal.Year > (dtToday.getFullYear()+EndYear))
		{
		    strCell = GenCell(j, false, DisableColor, true); 
		}
...
function NewCssCal(pCtrl, pFormat, pScroller, pShowTime, pTimeMode, pShowSeconds)
{
	...
	{
		//Cal.Format = "MMDDYYYY";
		Cal.Format = "YYYYMMDD";//MySQL date format...think of changing this for MSSQL at the time of deployement
	}
